...
os.system("hadoop fs -mkdir /user/s1155170000/hw3/2d")
os.system("hadoop fs -mkdir /user/s1155170000/hw3/2d/0")
os.system("hadoop fs -put part-00000 /user/s1155170000/hw3/2d/0/")

def check_converge(iter) :
    os.system("hadoop fs -get /user/s1155170000/hw3/2d/" + str(iter))
    cur_centroids = None
    with open(str(iter) + "/part-00000") as f :
        cur_centroids = np.array([list(map(float, vector.split(','))) for vector in f.readlines()])

    os.system("hadoop fs -get /user/s1155170000/hw3/2d/" + str(iter-1))
    prev_centroids = None
...
while (not check_converge(iter)) if iter > 0 else True :
    if os.system("hadoop jar /usr/lib/hadoop-mapreduce/hadoop-streaming.jar -files hdfs://dicvmc2.ie.cuhk.edu.hk/user/s1155170000/hw3/2d/" + str(iter) + 
                 "/part-00000 -file mapper.py -mapper \"python3 mapper.py\" -file reducer.py -reducer \"python3 reducer.py\" -input /user/s1155170000/data/train_img -output /user/s1155170000/hw3/2d/"
                 + str(iter+1)) != 0 :
